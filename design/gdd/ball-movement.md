...
> **Status**: Designed, revised after `/design-review` 2026-09-22 (full mode: game-designer, systems-designer, qa-lead, godot-specialist, creative-director; 6 blocking items resolved — F5 domain/guard, `OMEGA_MAX`/`BALL_LAG_TAU` cross-knob check, gate-policy ratification, `wrap`→`wrap_angle`, tube-track.md F9 cross-file edit, Player Fantasy/Rule 5 reconciliation; 13 important items and nice-to-haves deferred to a follow-up pass); pending re-review and the device spike BM-1..BM-6
> **Author**: user + agents
...
13. **Testing seam.** `BallCore` is a `RefCounted` with no engine calls; `BallMath` is a stateless static class holding the pure functions (F1's step, F2's `speed` and `S`, `wrap_angle`, F5); `BallConfig` is a `Resource` with every value in Tuning Knobs and a `validated(log_sink)` method that returns a clamped copy (a NaN, infinite or out-of-range `dt_max` given to the core falls back to 0.1 with one `KNOB_CLAMPED`; an unrecognized `MAPPING_MODE` falls back to POSITION with one). `validated()` also runs one **derived check** after the individual knob clamps: it computes `T_DODGE_180 = BallMath.T(PI, 0.05)` from the (already-clamped) `OMEGA_MAX` and `BALL_LAG_TAU` and, if it exceeds `T_DODGE_180_MAX` (1.14 s; Tuning Knobs, Knob Interactions), raises `OMEGA_MAX` to the smallest value that brings it back to the ceiling (capped at `OMEGA_MAX`'s own safe-range maximum, 4.0) and logs one `KNOB_CLAMPED`. This catches combinations where each knob is individually legal but the pair is not (for example `OMEGA_MAX` 2.75 with `BALL_LAG_TAU` 0.12, which independently pass their own ranges but together give `T_DODGE_180` 1.249 s). `BallCore` takes the validated `BallConfig`, the injected `dt_max` (Run State's `DT_MAX`) and a `log_sink` (a `Callable(level, code, message)`; the driver's sink rate-limits). Events in: `reset()`, `on_resumed()`, `step(dt_eff, steer, valid, input_source)`. Read-only test getters include `phi`, `phi_anchor`, `w` and `t_run` in addition to the published state of Rule 8. Tests build a fresh core each.
...
| Pattern & Difficulty | out | `speed(t_run)` curve, `V_MAX`, `OMEGA_MAX`, `STEER_ARC` | Its dodge budget (`T_dodge_worst`) is built from these; the reset glide (Rule 5) requires `s` = 0 to at least 11 u hazard-free |
...
Pattern & Difficulty and Tube Track F9 consume `T_DODGE_180 = T(PI, 0.05)` = **1.064 s**, the authoritative value (the 1.05 s assumption plus 0.017 s of lag; with `eps` 0.1 it is 1.022 s). This continuous value, not F1's frame-quantized example (1.0667 s at 60 Hz), is what other GDDs read. `T(PI/2, 0.1 * PI/2)` = 0.4718 s. F9 (`T_dodge_180` + 0.25 reaction + about 0.11 s latency = about 1.42 s against 1.5 s, a margin of about 0.08 s) stays valid while `T_dodge_180 <= T_DODGE_180_MAX` = **1.14 s** (derived from Tube Track's `T_VIS_MIN` default 1.5 s minus the 0.25 s reaction minus this 0.11 s latency term; re-derive both if either changes). The 0.11 s latency is the input-pipeline contribution *not* already inside `T_DODGE_180`'s own `tau * ln(K/eps)` term: about 0.06 s of sensor-plus-frame latency (0.01 s sensor, one poll frame, about two display frames, F5b) plus Tilt Input's own `FILTER_TAU` settling (about 0.05 s) layered in series ahead of `BALL_LAG_TAU`; adding F5b's combined "software response" or "end to end" figures instead would double-count `BALL_LAG_TAU`, which is already inside `T_DODGE_180`. Like every other latency figure here, 0.11 s is a guess pending BM-1's on-device measurement. Because no single pair of `OMEGA_MAX`/`BALL_LAG_TAU` boundary values keeps `T_DODGE_180 <= T_DODGE_180_MAX` safe in isolation (for example `OMEGA_MAX` 2.75 with `BALL_LAG_TAU` 0.12 each pass their own Tuning Knobs range but together give 1.249 s), the ceiling is enforced dynamically by `BallConfig.validated()`'s derived check (Rule 13), not by a static per-knob boundary.
...
| `BALL_LAG_TAU` | 0.06 s | 0 to 0.12 | The "slight inertia" and the software response time (F1, F5b): 63% response 0.118 s with Tilt Input's filter | Mechanical, passes sensor noise through | Mushy, breaks the 0.13 s software budget; combined with a low `OMEGA_MAX` can push `T_DODGE_180` over `T_DODGE_180_MAX` (F5a), where `validated()`'s derived check raises `OMEGA_MAX` to compensate |
| `OMEGA_MAX` | 3.0 rad/s | 2.75 to 4.0 | The fastest the ball can turn: `T_DODGE_180` 1.064 s (F5a), the sweep per step (0.3 rad at `DT_MAX`) | A 180 degree dodge is too slow, F9's `T_VIS_MIN` fails; the safe floor depends on `BALL_LAG_TAU` (F5a's derived check), not a fixed number | Dodges too easy, sweep per step 0.4 rad, harder contact tests |
...
- `OMEGA_MAX` and `BALL_LAG_TAU` together set `T_DODGE_180`; F9 stays valid while it is at most `T_DODGE_180_MAX` = 1.14 s (F5a). No single pair of boundary values from each knob's own safe range keeps this true in isolation (`OMEGA_MAX` 2.75 with `BALL_LAG_TAU` 0.12, both individually legal, give 1.249 s), so this is enforced as a derived check in `BallConfig.validated()` (Rule 13) rather than as two independent range checks: at `BALL_LAG_TAU` 0.12 the check needs `OMEGA_MAX` at least about 3.08 rad/s, not the previously-assumed 2.75. `BALL_LAG_TAU` and Tilt Input's `FILTER_TAU` add in series to the latency budget.
...
- **AC-12 [C]** (R5) Steer 1 to snap, then three times {`on_resumed()`, step 0.5, steer 1 to snap}: after chain 1 `phi` is 3 PI / 2, after chain 2 `phi` is 2 PI (1e-12) with no shift (the boundary `|phi| > 2 PI`); in chain 3 the shift fires on the first frame past 2 PI, `theta` equals `wrap_angle(unshifted)` to 1e-9, `omega` is computed before the shift, and `phi_anchor - phi` is unchanged; end state `theta` 1.5707963, `phi_anchor` -PI/2 (1e-9).
- **AC-13 [C]** (edge) Steer +1 and -1 each end with `theta == -PI` and `theta` stays in `[-PI, PI)`; seam frame: steer 0.95 to snap, `on_resumed()`, step 0.5, then steer 1: the 4th step crosses with `theta_prev` 3.134513, `theta` -3.098672, `omega` 3.0 (1e-6) and `wrap_angle(theta - theta_prev) / dt == omega`; the mirror gives -3.0.
...
- **AC-19b [K]** (R13, F5a, the derived cross-knob check) `OMEGA_MAX` 2.75 with `BALL_LAG_TAU` 0.12 (each individually inside its own safe range from AC-19, so neither triggers a per-knob clamp): `validated()` raises `OMEGA_MAX` to about 3.080 rad/s (the smallest value bringing `T(PI, 0.05)` to `T_DODGE_180_MAX` 1.14 s) with one `KNOB_CLAMPED`; `OMEGA_MAX` 2.75 with `BALL_LAG_TAU` 0.06 (default) needs no correction and logs nothing; a pair whose required correction would exceed `OMEGA_MAX`'s own safe-range maximum (4.0) clamps to 4.0 with one `KNOB_CLAMPED` (this pair is not reachable within the current safe ranges, but the clamp exists so the check cannot silently return an out-of-range knob).
...
**Gate policy.** AC-1..24 are Logic, BLOCKING. AC-25 is a CI-lint gate whose scripts and self-tests exist before the first Ball Movement story is Done. AC-27's static part is BLOCKING; AC-28 is ADVISORY. AC-29 is BLOCKING for the epic; AC-31 is BLOCKING for its story once the harness exists; AC-26, AC-30 and AC-32 are deferred. BM-1 and BM-3 are BLOCKING for first-playable (qa-lead and technical-director): coding-standards.md rates Visual/Feel ADVISORY by default, but these two are the sole falsification test for a stated pillar (BM-1 for Pillar 4's "any normal holding posture," BM-3 for Pillar 2's "never to randomness or bad luck") and a failure forces cross-system retuning (`OMEGA_MAX`, `tau`, `FILTER_TAU`, which also moves Tube Track F9's dodge budget and the reset-glide lead-in), not presentation polish — a documented exception under the escalation criterion added to coding-standards.md's Test Evidence table (creative-director designates, producer ratifies; `/design-review` 2026-09-22). BM-2 is BLOCKING only for locking the tuning defaults (`OMEGA_MAX`, `BALL_LAG_TAU`), ADVISORY for the first-playable build's existence: the GDD's own text treats a miss as "a tuning finding," which is inconsistent with gating the build's existence on it. BM-4 and BM-6 are ADVISORY, BM-5 blocks locking the `MAPPING_MODE` default. Owners: the epic's stories (AC-29, AC-30), Run State (`run_time` conformance), Tube Track (the AC-25 script), Tilt Input (AC-41 fixtures).

**Strategic note on margins (user decision, 2026-09-22, revisit only if BM-1/BM-2 miss):** `V_MAX` stays at 25 u/s for now rather than pre-emptively lowering it before the device spike — every default here is a guess either way. But the fairness chain this depends on (hazard/tube contrast headroom, Tube Track's `T_vis` margin, `T_DODGE_180`'s margin against `T_DODGE_180_MAX`) is thin and untested on a real phone (F5a, tube-track.md F9). If BM-1 or BM-2 miss after the spike, the default response is lowering `V_MAX` (toward about 22 u/s), not loosening Tube Track's `T_VIS_MIN` floor or this GDD's `T_DODGE_180_MAX` — loosening a validator is the easy path under deadline pressure and is exactly the path that would quietly erode Pillar 2.
